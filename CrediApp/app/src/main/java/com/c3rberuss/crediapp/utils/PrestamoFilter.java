...
public class PrestamoFilter extends Filter {

    PrestamoAdapter adapter;
    List<Prestamo> filterList;

    public PrestamoFilter(PrestamoAdapter adapter, List<Prestamo> filterList) {
    ...
    protected FilterResults performFiltering(CharSequence constraint) {
        ...
        if(constraint != null && constraint.length() > 0)
        {
            ...
            for (int i=0;i<filterList.size();i++)
            {
                ...
                {
                    if(!clientesFiltrados.contains(filterList.get(i))) {
